...
import type { AttributeRendererArgs, RenderContext, SurveyDocOptions } from './types'
...
const walkEntityChildrenGrid = async <T>(
  ...
  for (const def of Surveys.getNodeDefChildrenSorted({
    survey,
    nodeDef: entityDef,
    cycle,
  ...
  for (let y = 0; y < maxY; y++) {
    ...
    for (let x = 0; x < maxX; x++) {
      ...
      const limits = renderer.getGridCellLimits?.(maxX, w)
      let content: T[]
      if (NodeDefs.isEntity(nodeDef)) {
        content = await walkEntityDef(renderer, nodeDef as NodeDefEntity, context, depth + 1, parentEntityNode)
      } else {
        ...
        const args: AttributeRendererArgs = { nodeDef, context, depth, node: childNode, limits }
        content = await renderer.renderAttribute(args)
      }
